...
import { Program, ProgressMeeting, User, UserGroup, UserProgramRenewal } from "@prisma/client";
...
export const useDetailStudentViewModel = () => {
  ...
  const [selectedGroupMemberId, setSelectedGroupMemberId] = useState<
    string | null
  >(null);
    const { data: renewalData, mutate: mutateRenewal } =
      useSWR<RenewelResponse>(
        student_id
          ? `/api/admin/student/${student_id}/showRenewel`
          : null,
        fetcher
      );
...
const studentRenewals = Array.isArray(renewalData?.data)
  ? renewalData.data
      ...
      .sort(
        (a, b) => dayjs(a.renew_date).valueOf() - dayjs(b.renew_date).valueOf()
      )
  : [];
...
function getProgramInfoForMeeting(meetingDate: string) {
  ...
  if (studentRenewals.length > 0) {
    ...
    result.programId =
      studentRenewals[0].old_program_id || filteredStudent?.program_id || null;
