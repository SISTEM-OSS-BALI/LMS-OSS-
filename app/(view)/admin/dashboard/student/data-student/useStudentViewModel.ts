...
export const useStudentViewModel = () => {
  ...
  const { data: meetingDataAll } = useSWR<MeetingResponse>(
    "/api/admin/meeting/show",
    fetcher
  );

  const { data: programDataAll } = useSWR<ProgramResponse>(
      ...
      studentDataAll?.data?.map((student) => {
        ...
        const meetings =
          meetingDataAll?.data?.filter(
            (meeting) => meeting.student_id === student.user_id && meeting.absent === true
          ) ?? [];
  ...
  return { mergedStudent, studentDataAll };
};
