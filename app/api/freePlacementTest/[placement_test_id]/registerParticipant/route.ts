...
export async function POST(
  ...
  try {
    ...
    const { placement_test_id } = params;
    ...
    let session = await prisma.placementTestSession.findFirst({
      where: {
    ...
    if (!session) {
      session = await prisma.placementTestSession.create({
        data: {
          placementTestId: placement_test_id,
          sessionDate: today.toDate(),
        },
    ...
    const existingParticipant = await prisma.placementTestParticipant.findFirst(
      {
        where: {
    ...
    const newParticipant = await prisma.placementTestParticipant.create({
      data: {
        sessionId: session.session_id,
        name: fullName,
        email,
    ...
    return NextResponse.json({
      status: 201,
      error: false,
      message: "Successfully registered for the placement test",
      data: newParticipant,
    });
  } catch (error) {
